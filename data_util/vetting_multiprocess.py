...
def vetRecord(record_set):

  print "vetting a record ..."
  ...
  for record in record_set:
    for v in record['needs_vetting']:
      ...
      record_string = 'https://search.idigbio.org/v2/view/records/' + str(v) 
      idb = http.request('GET', record_string )
      if 200 == idb.status:

        idigbio = json.loads(idb.data)       

        if 'dwc:stateProvince' in idigbio['data']:
          if idigbio['data']['dwc:stateProvince'].lower() in record['states']:
            
            score = score + 1
            matched_fields.append("State Name")

        if 'dwc:specificEpithet' in idigbio['data'] and 'species' in record:
          if idigbio['data']['dwc:specificEpithet'] in record['species']:

            score = score + 1
            matched_fields.append("Species => Specific Epithet")

        if 'dwc:genus' in idigbio['data'] and 'genus' in record:
...
if __name__ == '__main__':
  ...
  data_chunk_size  = int(mp.cpu_count() * 1)

  print "pulling " + str(data_chunk_size) + " records "
 
  chunks = []
  cursor = db.pubIndexV2.find({ '$and': [ { 'needs_vetting': {'$exists': True}}, 
                                        { 'needs_vetting': { '$ne': []}},
                                        { 'vetted': { '$exists': False }} 
  ...
  for record in cursor:
    ...
    if 'needs_vetting' in record:
      #vettingCount = len(record['needs_vetting'])
      #print "We need to match " + str(vettingCount) + " IDB UUIDS "
  ...
  result = pool.map( vetRecord, [chunks[x:x + 1] for x in range(0, len(chunks), 1)] )
  pool.close()

  print "used cores: " + str(mp.cpu_count())
  print "End of multiprocess script: " + str(format(sum( result ) ) ) + " results process"
